"""
Django settings for MedixScan project.
"""
...
DATABASES = {
    'default': dj_database_url.config(
        default=config('DATABASE_URL', 
                      default=f"postgresql://{config('DB_USER', default='medixscan_user')}:"
                             f"{config('DB_PASSWORD', default='password')}@"
                             f"{config('DB_HOST', default='localhost')}:"
                             f"{config('DB_PORT', default='5432')}/"
                             f"{config('DB_NAME', default='medixscan_db')}")
    )
}
...
# Cache Configuration - Simplified for deployment
CACHES = {
    'default': {
...
LOGGING = {
    ...
    'handlers': {
        'file': {
            ...
            'filename': BASE_DIR / 'logs' / 'medixscan.log',
            'formatter': 'verbose',
        },
        'console': {
            'level': 'DEBUG',
...
# Session Security (Healthcare Compliance)
SESSION_COOKIE_SECURE = not DEBUG
SESSION_COOKIE_HTTPONLY = True
SESSION_COOKIE_AGE = config('SESSION_TIMEOUT', default=1800, cast=int)  # 30 minutes
...
ALLOWED_FILE_EXTENSIONS = config('ALLOWED_FILE_TYPES', default='pdf,jpg,jpeg,png,docx').split(',')

# Healthcare Data Retention (HIPAA Compliance)
DATA_RETENTION_DAYS = 2555  # 7 years as per HIPAA requirements

# Audit Logging for HIPAA Compliance
HIPAA_AUDIT_ENABLED = config('HIPAA_AUDIT_LOG', default=True, cast=bool)
